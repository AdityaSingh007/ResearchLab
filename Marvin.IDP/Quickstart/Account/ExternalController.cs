    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            if (user == null)
            {
                ...
                var returnUrlAfterRegistartion = Url.Action("Callback", new { returnUrl = result.Properties.Items["returnUrl"] ?? "~/" });

                var continueWithUrl = Url.Action("RegisterUser", "UserRegistration", 
                    new { returnUrl = returnUrlAfterRegistartion, provider = provider, providerUserId = providerUserId });
        ...
        private (User user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user = _marvinUserRepository.GetUserByProvider(provider, providerUserId);//_users.FindByExternalProvider(provider, providerUserId);
